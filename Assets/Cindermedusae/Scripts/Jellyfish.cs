...
public class Jellyfish : MonoBehaviour {
	...
	public void AnimateMesh()
	{
		...
		{
			vertices[index] += (Mathf.Sin(vertices[index].y / HeadR * Mathf.PI + Time.time * 4) * normals[index] * 3f);
		}
	...
	{
		mesh = new Mesh {name = "CinderMedusae2"};
		CombineInstance[] combine = new CombineInstance[NumTentacles + 2];
		...
		for (int segment = 0; segment <= Nsegments; ++segment)
		{
			...
			for (int side = 0; side <= Nsides; ++side)
			{
				...
				if (side % ((float)Nsides/NumTentacles) < 1 && segment == Nsegments/2 + TentacleYOffset)
				{
					...
					combine[tentacleIndex].mesh = tmesh;
					combine[tentacleIndex].transform = ttransform;
					tentacleIndex++;
				}
...
//		}	  
		...
		bodyMesh.normals = normals.ToArray();
		
		//mesh.uv = meshUV;
		var btransform = new Matrix4x4();
		btransform.SetTRS(Vector3.zero, Quaternion.identity, Vector3.one);
		combine[tentacleIndex].mesh = bodyMesh;
		combine[tentacleIndex].transform = btransform;
